...
while True:
    ...
    for box in results[0].boxes:
        ...
        if class_name.lower() == "cellphone":
            detected_closed_eyes = True

    # -----------------------------
    # Send ntfy notification
    # -----------------------------
    if detected_closed_eyes and not closed_eyes_alert_sent:

        send_ntfy(
            "Cellphone usage detected by the camera. "
        )

        closed_eyes_alert_sent = True

    # Reset after closed_eyes disappears
    if not detected_closed_eyes:
        closed_eyes_alert_sent = False
